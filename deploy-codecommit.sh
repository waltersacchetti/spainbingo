...
# Script para desplegar desde CodeCommit a la EC2
# Uso: ./deploy-codecommit.sh [OPCIÓN]
...
check_repo_status() {
    show_info "Verificando estado del repositorio CodeCommit..."
    
    # Verificar si el repositorio existe
    if ! aws codecommit get-repository --repository-name $REPO_NAME --region $REGION &> /dev/null; then
        show_error "Repositorio '$REPO_NAME' no existe en CodeCommit"
        show_info "Ejecuta: ./setup-codecommit.sh create-repo"
        exit 1
    fi
    ...
    show_success "Repositorio encontrado: $REPO_URL"
}

# Desplegar desde CodeCommit
deploy_from_codecommit() {
    load_instance_info
    check_aws_cli
    check_repo_status
    
    show_info "Desplegando desde CodeCommit..."
    ...
    aws ssm send-command \
        ...
        --parameters "commands=[
            ...
            \"echo '=== Actualizando desde CodeCommit ==='\",
            \"git status\",
            \"git fetch origin\",
            ...
            \"echo 'Despliegue desde CodeCommit completado'\"
        ]" \
...
deploy_commit() {
    ...
    if [ -z "$commit_hash" ]; then
        ...
        echo "Uso: ./deploy-codecommit.sh commit [HASH]"
        exit 1
    fi
...
show_repo_info() {
    ...
    show_info "Información del repositorio:"
    
    # Obtener información detallada
    aws codecommit get-repository \
        --repository-name $REPO_NAME \
        --region $REGION \
        --query 'repositoryMetadata.{Name:repositoryName,Description:repositoryDescription,CloneUrl:cloneUrlHttp,DefaultBranch:defaultBranch,LastModifiedDate:lastModifiedDate}' \
        --output table
    
    echo ""
    show_info "URL del repositorio:"
    echo "https://$REGION.console.aws.amazon.com/codesuite/codecommit/repositories/$REPO_NAME"
}

# Función para mostrar ayuda
show_help() {
    echo "Uso: ./deploy-codecommit.sh [OPCIÓN] [PARÁMETRO]"
    echo ""
    echo "Opciones:"
    echo "  deploy          - Desplegar desde CodeCommit (rama principal)"
    ...
    echo ""
    echo "Ejemplos:"
    echo "  ./deploy-codecommit.sh deploy"
    echo "  ./deploy-codecommit.sh branch develop"
    echo "  ./deploy-codecommit.sh commit abc123"
    echo "  ./deploy-codecommit.sh status"
    echo "  ./deploy-codecommit.sh rollback"
    echo ""
    echo "Flujo recomendado:"
    echo "1. git add . && git commit -m 'Cambios' && git push"
    echo "2. ./deploy-codecommit.sh deploy"
    echo "3. ./deploy-codecommit.sh status"
}
